export default function DifficultyPicker({ value, onChange, availableDifficulties }) {
    ...
    <fieldset className="text-center">
      ...
      <div className="inline-flex rounded-2xl bg-[#68659A] text-white overflow-hidden">
        {options.map((opt) => {
          ...
          const disabled = !availableDifficulties.includes(opt);
          
          return (
            <label
              key={opt}
              className={`px-6 py-3 flex items-center gap-2 select-none
              ...
              <span className="rounded-full border-2 w-3 h-3 grid place-items-center">
                {active && !disabled && <span className="w-1.5 h-1.5 rounded-full bg-white" />}
              </span>
              {opt}
            </label>
          );
