              ...
              <div className="rounded-2xl overflow-hidden bg-[#1e1e1e] border border-gray-800 flex flex-col">
                ...
                <div className="bg-[#2d2d2d] border-b border-gray-700">
                  <div className="flex items-center gap-1 px-4 pt-3">
                    {testsWithVerdict.length > 0 ? (
                      testsWithVerdict.map((tc, idx) => (
                        <button
                          key={tc.id}
                      ...
                      <span className="px-4 py-2 text-sm text-gray-200">
                        No test cases
                      </span>
                    )}
                  </div>
                ...
                <div className="p-4 bg-[#1e1e1e] space-y-4">
                  {testsWithVerdict[activeTab] ? (
                    <>
                      <div>
                        <div className="text-white text-sm mb-1">Input:</div>
                        <div className="bg-[#2d2d2d] p-3 rounded text-gray-100 text-sm font-mono whitespace-pre-wrap break-words">
                          {testsWithVerdict[activeTab].raw &&
                          typeof testsWithVerdict[activeTab].raw.input === "object"
                            ? formatObjectAsLines(
                                testsWithVerdict[activeTab].raw.input
                              )
                            : formatScalarDisplay(
                                testsWithVerdict[activeTab].raw.input
                              )}
                      ...
                      <div>
                        ...
                        <div className="bg-[#2d2d2d] p-3 rounded text-gray-100 text-sm font-mono whitespace-pre-wrap break-words">
                          {testsWithVerdict[activeTab].outputDisplay}
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div
                          className={`p-3 rounded text-sm font-mono whitespace-pre-wrap break-words ${
                            testsWithVerdict[activeTab].status === "pass"
                              ? "bg-green-900/40 text-green-50"
                              : testsWithVerdict[activeTab].status === "fail"
                              ? "bg-red-900/40 text-red-50"
                              : "bg-[#2d2d2d] text-gray-100"
                          }`}
                        >
                          {testsWithVerdict[activeTab].userOutput ||
                            "(no output recorded)"}
                        </div>
                      </div>
                      ...
                      <div className="text-sm">
                        ...
                        >
                          {testsWithVerdict[activeTab].status.toUpperCase()}
                        </span>
                      </div>
                    </>
                  ) : (
